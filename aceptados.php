...
<body class="grid-container">
  ...
  <main class="main">
    <h3>Ver los archivos de los alumnos aceptados:<br><br></h3>
    <form class="formulario" method="POST">
      <h4>Matricula:
      <?php 
        $sql="SELECT DISTINCT carrera FROM archivosaceptados";
...
<?php
  if (isset($_POST['buscar'])) {
    ...
    $query = "SELECT * FROM archivosaceptados WHERE carrera LIKE '%".$carrera."%'";

    $query_searched = mysqli_query($conexion,$query);
    $count_results = mysqli_num_rows($query_searched);
    ?>
    <table border="1" >
      <p>A L U M N O S  A C E P T A D O S   2 0 2 1  T E S I</p>
    <tr>
      <th>CARRERA</th>
      <th>MATRICULA</th>
      <th>ARCHIVOS</th> 
    ...
    if ($count_results > 0) {
        while ($mostrar = mysqlI_fetch_array($query_searched)) {
          $ruta='archivos/'.$mostrar['matricula'].'/'.$mostrar['archivo'];
            ?>
            <tr>
...
<?php 
    $sql="SELECT * from archivosaceptados";
    $result=mysqli_query($conexion,$sql);
while($mostrar=mysqli_fetch_array($result)){
  $ruta='archivos/'.$mostrar['matricula'].'/'.$mostrar['archivo'];
     ?>
