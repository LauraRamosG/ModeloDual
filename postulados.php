...
<body class="grid-container">
  ...
  <main class="main">    
     <h3>Ver archivos de alumnos postulados:<br><br></h3>
    <form class="formulari" method="POST">
      <h4>Matricula:
      <?php 
        $sql="SELECT DISTINCT matricula FROM archivosalumnos";
...
<?php
  if (isset($_POST['buscar'])) {
    ...
    $query = "SELECT * FROM archivosalumnos WHERE matricula LIKE '%".$matricula."%'";
    $query_searched = mysqli_query($conexion,$query);
    $count_results = mysqli_num_rows($query_searched);
    ?>
    <table border="1" >
      <p>A R C H I V O S  P O S T U L A D O S   2 0 2 1  T E S I</p>
    <tr>
      <th>MATRICULA</th>
      <th>ARCHIVO</th>
    </tr>
...
<?php
    $sql="SELECT * from archivosalumnos";
    $sql="SELECT * from archivosalumnos";
    $result=mysqli_query($conexion,$sql);
while($mostrar=mysqli_fetch_array($result)){
     $ruta='archivos/'.$mostrar['matricula'].'/'.$mostrar['archivo'];
