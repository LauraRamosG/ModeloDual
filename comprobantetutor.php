<?php
    ...
    if(isset($_POST["submit"])){
        ...
        $sentencia = $conn->prepare('SELECT * FROM datostutores WHERE id_trabajador = ?;');
        $sentencia->execute([$id_trabajador]);
        $datos = $sentencia->fetch(PDO::FETCH_OBJ);
        $sentenciaarchivo = $conn->prepare('SELECT * FROM archivostutores WHERE id_trabajador = ?;');
        $sentenciaarchivo->execute([$id_trabajador]);
        ...
        if($datos === false){
            ...
            $pname = $id_trabajador."-".rand(1,1000)."-".$_FILES["file"]["name"];
            $tname = $_FILES["file"]["tmp_name"];
            $uploads_dir = 'archivos';
            move_uploaded_file($tname, $uploads_dir.'/'.$pname);
            if(!empty($_FILES['file']['name'])){
                $sql = "INSERT into archivostutores(id_trabajador, archivo) VALUES('$id_trabajador','$pname')";
                ...
                try {
                    ...
                    $subject = 'Oficio de Asignacion';
                    $body = 'El nombre del tutor: ' . $_SESSION['nombres'] . ' ' . $_SESSION['apellidos'] . ', con id_trabajador: ' . $_SESSION['id_trabajador'] . '. El oficon de Asignacion es el siguiente: ';
                    $fromemail = 'modelodual21@gmail.com';
                    $fromname = 'Eduacion DUAL';
            ...
            window.location.assign("comprobantetutor.php");
            </script>';
            <?php
        }
    }
...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="css/comprobantetutor.css"/>
    ...
    <body class="grid-container">
            ...
            <div class="main">
                ...
                <form class="formulario" method="POST" enctype="multipart/form-data">
                    <div>
                    <h3>Oficio de Asignacion</h3>
                    <input class="controls" type="text" name="matricula" placeholder="Ingrese su id_trabajador"/>
